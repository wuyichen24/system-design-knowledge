...
| Pattern Name | Diagram | Description |
| ---- | ------ | ---- |
| Two-Phase Commit (2PC) |  | <ul><li>**Commit-Request/Voting Phase**<ul><li>Each participants execute the transaction and send back the aggreement message (the transcation is successful) or the abort message (the transcation is fail) to the the coordinator.</ul><li>**Commit/Completion Phase**<ul><li>**Success**: If the coordinator received an agreement message (success) from all participants:<ul><li>The coordinator sends a commit message to all the participants.<li>Each participant commits the transcation and releases all the locks.</ul><li>**Failure**: If any participant sent a abort message to the coordinator:<ul><li>The coordinator sends a rollback message to all the participants.<li>Each participant undos the transaction and release all the locks.</ul></ul></ul> |
| Three-Phase Commit (3PC) |  |  |
